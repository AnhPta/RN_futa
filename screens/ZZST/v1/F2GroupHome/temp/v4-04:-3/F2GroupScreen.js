...
class F2GroupScreen extends Component {
  ...
  submitChangeText (event) {
    let index = findIndex(this.state.listPackage, function (o) {
      return o.barcode === event.nativeEvent.text.trim() && o.type_package_app === 1
    })
  ...
  confirmF2Group () {
    ...
    let data = {
      ...
      "to_department_id": this.state.listPackage[index].to_department_id,
          ...
          <View style={{ backgroundColor: '#eeeeee', flex: 1 }}>
            ...
            <ScrollView>
              <View style={{ paddingTop: 10 }}>
                {
                  this.state.listPackage.map((item, index) => {
                    if (item.checked == 1 && item.type_package_app === 1) {
                      return (
