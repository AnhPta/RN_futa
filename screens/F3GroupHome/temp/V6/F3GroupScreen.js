...
import { StyleSheet, Text, View, Button, TouchableOpacity, ScrollView, TextInput, Modal, Alert, Dimensions , Keyboard, StatusBar} from 'react-native';
...
import { f3Change, f3Set } from './redux/action';
import { findIndex, forEach, chain, groupBy, map, filter, set } from 'lodash'
...
class F3GroupScreen extends Component {
    ...
    axios.post('local_f3_get_list_package_to_group', data).then(response => {
      ...
      if (response.Status == 1) {
        ...
        if (!this.isCancelled) {
          ...
          this.props.f3Set(listPackageTemp)
          // Lưu
          this.setState({
            ...this.state, ...{
              loading: false,
  ...
  selectPackage (indexGroupBy, index) {
    ...
    // let listPackageTemp = this.state.selectPackage;
    // listPackageTemp[indexGroupBy].items[index].checked = ! listPackageTemp[indexGroupBy].items[index].checked;
    // set(this.state, 'listPackage[' + indexGroupBy + '].items[' + index + '].checked', 1);
    // => 4
    // this.setState({
    //   ...this.state, listPackage: listPackageTemp
    // });
  ...
  openModal () {
    ...
    if (!flag) {
      Alert.alert(
        'Thông báo',
        'Vui lòng chọn gói F1, F2 cần nhóm'
      );
      return
    }

    // Kiểm tra đã chọn chi nhánh tới
    if (!this.props.f3State.toDepartment.name) {
      Alert.alert(
        'Thông báo',
  ...
  confirmF2Group () {
    ...
    let data = {
      ...
      "to_department_id": this.props.f3State.toDepartment.id,
      ...
      <View style={styles.container}>
        ...
        <TouchableOpacity onPress={() => this.props.navigation.navigate('F3DepartmentGroupToForCurrentDepartmentScreen')}>
          <View style={{ height: 30, borderRadius: 5, marginLeft: 5, marginRight: 5, marginBottom: 10, paddingLeft: 10, borderWidth: 1, borderColor: '#ccc', justifyContent: 'center' }}>
            {
              this.props.f3State.toDepartment.name ?
              <Text>
                {this.props.f3State.toDepartment.name}
              </Text>
              :
              <Text style={{ color: '#ccc' }}>Chưa chọn nơi đến ...</Text>
            }
        ...
        <ScrollView>
          ...
          <View>
            {
              this.props.f3State.listPackage.map((groupBy, indexGroupBy) => {
                return (
        ...
        <Modal
          ...
          <View style={{ backgroundColor: '#fff', flex: 1 }}>
            ...
            <View style={{ display: 'flex', flexDirection: 'row', paddingLeft: 10 }}>
              <Text>Đóng hàng đến: </Text>
              <Text style={{ fontWeight: 'bold' }}>{ this.props.f3State.toDepartment.name }</Text>
            </View>
            <ScrollView>
...
export default connect(mapStateToProps, mapDispatchToProps)(F3GroupScreen);
