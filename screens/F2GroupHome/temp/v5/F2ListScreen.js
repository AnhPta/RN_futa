...
class F2ListScreen extends Component {
  ...
  getF2Detail (item) {
    ...
    this.fetchF2Detail(item);
  }


  // Xem chi tiết F2 có gì:
  fetchF2Detail (item) {
    let data = {
      "f2_id": item.bill_id
    }
                              ...
                              <View style={{ height: 70, borderBottomWidth: 1, borderColor: '#F3F3F3', display: 'flex', flexDirection: 'row', alignItems:'center', marginRight: 10, marginLeft: 10, justifyContent: 'space-between' }}>
                                ...
                                <View style={{ display: 'flex', flexDirection: 'row', alignItems:'center', flex: 60 }}>
                                  <View style={{ display: 'flex', flexDirection: 'column', marginLeft: 5 }}>
                                    ...
                                    <Text style={{ marginTop: 4, fontSize: 13, color: '#4a4a4a' }}>{ item.quantity_total } kiện - { item.product_weight } kg</Text>
                                  </View>
                                </View>
        ...
        <Modal
          ...
          <View style={{ backgroundColor: '#fff', flex: 1 }}>
            ...
            <View style={{ height: 44, alignItems: 'center', justifyContent: 'space-between', display: 'flex', flexDirection: 'row', paddingLeft: 10, paddingRight: 10 }}>
              ...
              <Text style={{ fontWeight: 'bold', fontSize: 16 }}>Chi tiết - { this.state.item.barcode }</Text>
              <TouchableOpacity onPress={ () => this.confirmUnGroup() }>
                <View style={{ height: 44, paddingRight: 10, justifyContent: 'center', display: 'flex' }}>
                  <Text style={{ fontSize: 15, color: '#000' }}>Xả túi</Text>
                </View>
              </TouchableOpacity>
            </View>
            <ScrollView>
              ...
              {
                this.state.listPackageF1.map((item1, index) => {
                  return (
                    <TouchableOpacity key={item1.barcode}>
                      <View style={{ height: 70, borderBottomWidth: 1, borderColor: '#F3F3F3', display: 'flex', flexDirection: 'row', alignItems:'center', marginRight: 10, marginLeft: 10, justifyContent: 'space-between' }}>

                        <View style={{ display: 'flex', flexDirection: 'row', alignItems:'center', flex: 60 }}>
                          <View style={{ display: 'flex', flexDirection: 'column', marginLeft: 5 }}>
                            <Text style={{ fontSize: 15, fontWeight: 'bold' }}>{ item1.barcode }</Text>
                            <Text style={{ marginTop: 4, fontSize: 13, color: '#4a4a4a' }}>{ item1.product_name } - { item1.product_weight } kg</Text>
                          </View>
                        </View>

                        <View style={{ flex: 40, alignItems: 'flex-end', flexDirection: 'column' }}>
                          <Text style={{ fontSize: 12 }}>{ item1.time_wait }</Text>
                          <MaterialCommunityIcons style={{ marginTop: 8, color: item1.color_app }} name='checkbox-blank-circle' size={ 14 } />
                        </View>
